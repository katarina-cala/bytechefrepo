...
const WorkflowNodesPopoverMenu = ({
    ...
    const getFormattedClusterElementName = (clusterElementName: string, clusterElementType: string) => {
        ...
        // console.log('workflowDefinition', workflowDefinition.tasks);
        // console.log('clusterElementType', clusterElementType);
        ...
        // console.log('clusterElementNames', clusterElementNames);

        const existingClusterElements = clusterElementNames.flatMap((names: string[]) => {
            if (clusterElementType === 'tools') {
                //ode triba flatMap da bi array sa arrayima pribacilo u jedan array da se moze usporedit svaki element je li vec postoji ([createRow], [createRow] triba pribacit u [createRow, createRow] tako da se moze usporedit svaki element zasebno...to triba kod toolsa jer su oni nested a za ove druge ne tria)
                return names.filter((name: string) => name?.includes(clusterElementName));
            }
            return names?.includes(clusterElementName) ? [names] : [];
        });

        // const existingClusterElements = clusterElementNames.filter((name) => name?.includes(clusterElementName)); -> PRVOTNA VERZIJA KOJA NE FUNKCIONIRA ZA TOOLS

        // console.log('existingClusterElements', existingClusterElements);
    ...
    const handleClusterElementClick = (data: ClusterElementDefinitionBasic) => {
        ...
        // const updatedAgentData = {...clusterElementsData};
        ...
        // console.log('data', propertyMap[data.type as ElementTypeKeysType]);

        if (data.type === 'TOOLS') {
            // console.log('tools NAME', data.name);
            ...
            // console.log('model name', data.componentName);
        ...
        // console.log('workflow node name', sourceNode.data.workflowNodeName);
