...
const AIAgentNode = ({data, id}: {data: NodeDataType; id: string}) => {
    ...
    const [clusterElementsData, setClusterElementsData] = useState<AgentDataType>({
        chatMemory: null,
        model: null,
        rag: null,
        tools: [],
    ...
    // console.log('clusterElementTypeDefinition', clusterElementDefinition);
    // console.log(data.name, 'CLUSTER ELEMENT DATA', data.clusterElements);
    // console.log(data.name, 'agent data', agentData);
    ...
    // console.log('data from definition', data);
    // console.log('data from LOCAL STATE', clusterElementsData);
    ...
    const agentModelName = getElementDisplayName(clusterElementsData?.model);
    const agentMemoryName = getElementDisplayName(clusterElementsData?.chatMemory);
    const agentRagName = getElementDisplayName(clusterElementsData?.rag);
    const agentToolNames = clusterElementsData?.tools?.map((tool) => getElementDisplayName(tool)).filter(Boolean);

    // const agentModelName = clusterElementsData?.model?.label
    //     ? clusterElementsData?.model?.label.split('_')[0]
    //     : clusterElementsData?.model?.name.split('_')[0];

    // const agentMemoryName = clusterElementsData?.chatMemory?.label
    //     ? clusterElementsData?.chatMemory?.label.split('_')[0]
    //     : clusterElementsData?.chatMemory?.name.split('_')[0];

    // const agentRagName = clusterElementsData?.rag?.label
    //     ? clusterElementsData?.rag?.label.split('_')[0]
    //     : clusterElementsData?.rag?.name.split('_')[0];

    // const agentToolNames = clusterElementsData?.tools?.map((tool) =>
    //     tool.label ? tool.label.split('_')[0] : tool.name.split('_')[0]
    // );
