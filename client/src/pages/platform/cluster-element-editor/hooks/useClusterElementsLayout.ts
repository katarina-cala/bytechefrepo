...
const useClusterElementsLayout = () => {
    ...
    const canvasWidth = window.innerWidth - 80 - 120;
    ...
    const {allNodes, taskEdges} = useMemo(() => {
        ...
        if (rootClusterElementNodeData) {
            const nodePosition = rootClusterElementNodeData.metadata?.ui?.nodePosition || DEFAULT_NODE_POSITION;

            const rootClusterElementNode = {
                data: rootClusterElementNodeData,
                id: rootClusterElementNodeData.workflowNodeName,
                position: nodePositions[rootClusterElementNodeData.workflowNodeName] || nodePosition,
                type: 'workflow',
            };
